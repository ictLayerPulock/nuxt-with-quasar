<template>
  <q-select
    ...
    @update:model-value="$emit('costCenterSelect', cost_center)"
    dense
    outlined
    :rules="[(val) => val || 'Please select a cost income center']"
  >
...
async function filterFn(val, update) {
  ...
  update(() => {
    ...
    costCenterList.value = getList.value.filter(
